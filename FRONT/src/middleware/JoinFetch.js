...
const JoinFetch = createAsyncThunk("JoinSlice/JoinFetch", async (state) => {
  ...
  const JoinResult = await axios({
    ...
    .then((e) => {
      ...
      console.log(e);
      // e.data는 user_service에서 보낸 res.send의 응답이 담긴다!
      console.log(e.data);
      return e.data;
    })
    .catch((error) => {
...
const checkFetch = createAsyncThunk("joinSlice/checkFetch", async (id) => {
  const checkResult = await axios({
    method: "post",
    url: "http://localhost:80/idcheck",
    // 회원가입에서 입력한 값이 state에 담긴다
    data: id,
  })
    .then((e) => {
